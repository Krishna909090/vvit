...
| Method | Endpoint | Description | Data Stored/Updated | Tables Involved |
| :--- | :--- | :--- | :--- | :--- |
| POST | `/student/register` | Registers a new student application. | `name`, `email`, `phone`, `dob`, `address`, `courseType`, `preferences`, etc. | `Student` |
| POST | `/student/:studentId/pay-test-fee` | Marks the test fee as paid for a student. | `feePaid` (Student), Payment record | `Student`, `Payment` |
| GET | `/student/:studentId/hall-ticket` | Retrieves the generated hall ticket URL. | None (Read-only) | `Student` (reads `hallTicketUrl`) |
| POST | `/student/:studentId/upload-docs` | Uploads documents and updates branch preferences. | `marksheetUrl`, `tcUrl`, `casteUrl`, `pref1`, `pref2`, `pref3`, etc. | `Student` |
| POST | `/student/:studentId/pay-college-fee` | Processes the college admission fee payment. | `feePaid` (Student), Payment record | `Student`, `Payment` |
...
| POST | `/student/:studentId/select-exam` | Selects an exam slot and generates a hall ticket. | `examSlotId` (Student), `filled` (ExamSlot), `url`, `qrHash` (HallTicket) | `Student`, `ExamSlot`, `HallTicket` |
...
| DELETE | `/student/:studentId/document` | Deletes a specific uploaded document. | Sets specific document URL field to null. | `Student` |
...
| POST | `/admin/upload-results` | Bulk uploads exam results via CSV. | `examScore`, `isQualified` | `Student` |
...
| GET | `/admin/dashboard-stats` | Retrieves dashboard statistics. | None (Read-only) | `Student` (aggregates) |
| GET | `/admin/applications` | Lists all student applications with pagination and search. | None (Read-only) | `Student` |
| POST | `/admin/upload-applications` | Bulk uploads student applications via CSV. | Creates multiple Student records. | `Student` |
| GET | `/admin/certificates/:studentId` | Retrieves URLs of student certificates. | None (Read-only) | `Student` |
| POST | `/admin/mark-attendance` | Marks student attendance for an exam. | `examAttended`, `status` | `Student` |
| POST | `/admin/verify-allot` | Verifies documents and allots a branch seat. | `status`, `allottedBranch` (Student), `filledSeats` (Branch) | `Student`, `Branch` |
| POST | `/admin/change-branch` | Requests a branch change for a student. | `fromBranch`, `toBranch`, `reason`, `status` | `BranchChangeRequest` |
| POST | `/admin/approve-branch-change` | Approves a branch change request. | `status` (Request), `allottedBranch` (Student), `filledSeats` (Branch), Log entry | `BranchChangeRequest`, `Student`, `Branch`, `BranchChangeLog` |
...
| POST | `/admin/approve-cancellation` | Approves cancellation. | `status` (Request), `status` (Student), `filledSeats` (Branch) | `CancellationRequest`, `Student`, `Branch` |
| POST | `/admin/update-exam-score` | Updates a student's exam score manually. | `examScore`, `isQualified` | `Student` |
| GET | `/admin/download-documents/:studentId` | Downloads all student documents as a ZIP. | None (Read-only) | `Student`, `DiscountRequest` |
...
| POST | `/admin/update-admission` | Updates admission details (accommodation, fees). | `accommodationType`, `hostelId`, `transportRouteId`, `totalFee`, `paidFee` | `Student`, `Hostel`, `TransportRoute` |
| GET | `/admin/fee-stats` | Retrieves fee collection statistics. | None (Read-only) | `Student` (aggregates) |
...
| POST | `/admin/fee-structure` | Creates a fee structure for a program. | `amount`, `academicYear`, `programId`, `feeHeadId` | `FeeStructure` |
...
| POST | `/admin/verify-document/:studentId` | Updates verification status of a specific document. | `documentVerificationStatus` (JSON) | `Student` |
...
| POST | `/exam/slots/:studentId/book` | Books an exam slot for a student. | `examSlotId` (Student), `filled` (ExamSlot) | `Student`, `ExamSlot` |
| POST | `/exam/invigilators/login` | Logs in an invigilator. | None (Read-only check) | `InvigilatorCredential` |
| POST | `/exam/attendance/scan` | Marks attendance via QR scan. | `examAttended` (Student), `scannedAt` (AttendanceRecord) | `Student`, `AttendanceRecord` |
...
| POST | `/invigilator/scan-qr` | Scans a student's QR code to mark attendance. | `examAttended` (Student), `scannedAt` (AttendanceRecord) | `Student`, `AttendanceRecord` |
...
- **Student**: Core table storing all student application data, status, and foreign keys to other modules.
...
- **Student -> ExamSlot**: Links a student to their booked exam time.
- **Student -> Hostel/TransportRoute**: Links a student to their accommodation.
